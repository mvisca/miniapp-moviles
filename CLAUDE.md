...
**Verificación empírica (TASK-002-0)**: el scaffold generado hoy (`create-vite@latest -t react-ts --eslint`, 2026-07-30) trae ESLint `10.8.0`, no `9.x` como se documentaba antes de verificar — dato que corrige el número de versión en la fila «Linter» de §2, aunque esa fila no se edita aquí por estar fuera del alcance de esta task. El formato de config sigue siendo flat (`eslint.config.js`), confirmado, pero generado con los helpers `defineConfig`/`globalIgnores` de `eslint/config` (no un array plano de objetos): `globalIgnores(['dist'])` reemplaza lo que antes hubiera sido una entrada `{ ignores: [...] }` suelta en el array. Sin `.eslintignore`: las exclusiones van en ese `globalIgnores(...)` dentro del propio config, no en un archivo aparte — el formato `.eslintignore` sigue siendo legacy (ESLint ≤8) y las versiones actuales ni lo leen por defecto. No hace falta excluir explícitamente `CLAUDE.md`, `README.md` u otros documentos: el propio config ya scopea el lint por extensión (`files: ['**/*.{ts,tsx}']`), así que `eslint .` nunca llega a evaluar Markdown — no hay nada que agregar para eso.

**Nota adicional (fuera del alcance de ESLint pero detectada durante la misma verificación empírica)**: pnpm 11 dejó de leer `save-exact` desde `.npmrc` (solo mantiene ahí los ajustes de auth/registry); el pin exacto de versiones vía `pnpm add` ahora requiere `saveExact: true` en `pnpm-workspace.yaml` (añadido en esta task). `.npmrc` con `save-exact=true` se conserva porque sigue siendo válido para `npm install`, tal como documenta la fila «Versionado de dependencias» de §2.
